...
func InitializeAPI() (*server.EchoServer, error) {
	...
	adminService := admin.NewAdminService(repository, logRepository)
	...
	userService := user2.NewUserService(repository, logRepository, config)
	authService := auth2.NewAuthService(v, repository, userService, logRepository, config)
	...
	rbacMiddleware := middlewares.NewRbacMiddleware(config, userService)
